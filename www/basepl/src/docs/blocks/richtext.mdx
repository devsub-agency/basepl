...
<CommandPreview
  ...
  pnpmCommand={'npx @basepl/cli add BaseplRichText'}
/>
...
If not you will get an error in the `BaseplRichtext/Component.tsx` file. Because the BaseplImageType is not present.

## Richtext block configuraton

Here is the result in the admin panel and the pre-configured richtext block itself. You can change it to your needs in the `BaseplRichtext/config.ts`file.

<BlockPreview name="BaseplRichtext" imagePath="admin-button.png" />
...
The basepl richtext act as a wrapper for the richtext-lexical, to apply admin configurations. You can change it to your needs in the `BaseplRichtext/Component.tsx`.
...
## Preconfigured richtext options
